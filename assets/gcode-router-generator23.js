...
function generateRouterRasterGcode() {
  ...
  try {
    ...
    // ================= Header =================
    lines.push('; ============================================================');
    lines.push('; 🧠 Generated by CncAi — Router Raster Engraving');
    ...
    lines.push('; ✅ Completed Successfully by CncAi');
    lines.push('; ============================================================');

    const timeMin = totalLen / (feed || 1);
    const estNode = document.getElementById('estTime');
    if (estNode) estNode.innerHTML =
      `⏱️ تقدير وقت التشغيل: ${timeMin.toFixed(1)} دقيقة | ${moveCount} حركة`;

    return lines.join('\n');
  } catch (e) {
    console.error('❌ خطأ في توليد كود الراوتر Raster:', e);
    throw e;
  }
}
...
function generateRouterContourGcode() {
  ...
  try {
    ...
    // ================= Header =================
    lines.push('; ============================================================');
    lines.push('; 🧠 Generated by CncAi — Router Contour Milling');
    ...
    const estNode = document.getElementById('estTime');
    if (estNode) estNode.innerHTML =
      `⏱️ تقدير وقت القص: ${timeMin.toFixed(1)} دقيقة | طول المسار ${Math.round(totalLen)} مم`;
