// ===============================================================
// 🧠 CncAi — Router G-code Generator (Raster + Contour)
// الإصدار المحسن — 2025
// يحافظ على المنطق الأصلي مع تحسينات: Header / Footer / Precision / Home / Motor Off
// ===============================================================
...
function generateRasterGcode(scaleDown = false) {
  ...
  try {
    ...
    // 🧾 Header
    lines.push('; ============================================================');
    lines.push('; 🧠 Generated by CncAi — Advanced CNC Tool');
    lines.push('; Mode: Router | Pattern: Raster');
    lines.push('; Date: ' + new Date().toLocaleString());
    lines.push('; SafeZ: ' + safeZ + ' mm | Feed: ' + feed + ' mm/min');
    lines.push('; ============================================================');
    lines.push('G90 G21 G17');
    lines.push('G28 ; home all axes');
    lines.push('G0 Z' + safeZ.toFixed(precision));
    lines.push('M3 ; spindle on');
    lines.push('; --- Begin cutting path ---');
      ...
      for (let y = 0; y < previewCanvas.height; y += step) {
        ...
        for (let x = 0; x < previewCanvas.width; x += 2) {
          ...
          if (inside && !inContour) { segmentStart = x; inContour = true; }
            ...
            addSegmentPoints(rowPoints, segmentStart, x - 1, y, scaleX, scaleY, originX, originY,
              maxDepth, invertZ, useFixedZ, fixedZValue);
            inContour = false;
          }
        }
        if (inContour) {
          addSegmentPoints(rowPoints, segmentStart, previewCanvas.width - 1, y,
            scaleX, scaleY, originX, originY, maxDepth, invertZ, useFixedZ, fixedZValue);
        }
      ...
      for (let x = 0; x < previewCanvas.width; x += step) {
        ...
        for (let y = 0; y < previewCanvas.height; y += 2) {
          ...
          if (inside && !inContour) { segmentStart = y; inContour = true; }
            ...
            addVerticalSegmentPoints(colPoints, x, segmentStart, y - 1, scaleX, scaleY, originX, originY,
              maxDepth, invertZ, useFixedZ, fixedZValue);
            inContour = false;
          }
        }
        if (inContour) {
          addVerticalSegmentPoints(colPoints, x, segmentStart, previewCanvas.height - 1,
            scaleX, scaleY, originX, originY, maxDepth, invertZ, useFixedZ, fixedZValue);
        }
...
// ===============================================================
// 🧠 توليد G-code من نوع Contour للراوتر
// ===============================================================

function generateContourGcode() {
  if (!grayMat || !contour) throw new Error("لا توجد بيانات حواف لتوليد الكود");
  ...
  try {
    ...
    lines.push('; ============================================================');
    lines.push('; 🧠 Generated by CncAi — Advanced CNC Tool');
    lines.push('; Mode: Router | Pattern: Contour');
    lines.push('; Date: ' + new Date().toLocaleString());
    lines.push('; SafeZ: ' + safeZ + ' mm | Feed: ' + feed + ' mm/min');
    lines.push('; ============================================================');
    lines.push('G90 G21 G17');
    ...
    for (const cnt of contoursToUse) {
      ...
      let zStart = useFixedZ ? fixedZValue : -((255 - startGray) / 255.0) * maxDepth;
      if (invertZ) zStart = -zStart;

      lines.push(`G0 X${startX} Y${startY} Z${safeZ.toFixed(precision)}`);
      lines.push(`G1 F${feed.toFixed(0)}`);
      lines.push(`G1 Z${zStart.toFixed(precision)}`);
      ...
      for (let i = 2; i < data.length; i += 2) {
        ...
        lines.push(`G1 X${px} Y${py} Z${zVal.toFixed(precision)}`);
        totalLen += Math.hypot(x - x0, y - y0);
        x0 = x; y0 = y;
      }

      lines.push(`G1 X${startX} Y${startY} Z${zStart.toFixed(precision)}`);
      lines.push(`G0 Z${safeZ.toFixed(precision)}`);
    }
    ...
    document.getElementById('estTime').innerHTML =
      "⏱️ تقدير الوقت (Contour): " + timeMin.toFixed(1) + " دقيقة";
